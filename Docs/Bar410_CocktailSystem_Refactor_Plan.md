...
**ทุกข้อปิดแล้ว ยกเว้น D7 ที่รอ design เล่นทดสอบ** — แผนนี้เริ่มลงมือได้โดยไม่ติดคำถามค้าง
...
| **D7** | `MaxTolerance` ของ deviation หลังแก้ S1 | **คง `3` ตาม GDD §17.3 ไว้ก่อน — ⚠️ FLAG รอ design ยืนยัน** | ดู §10.3 |
...
### 10.3 D7 — `MaxTolerance` ⚠️ FLAG รอ Game Designer

**สถานะ: คงค่า `3` ตาม GDD §17.3 ไว้ก่อน แล้วรอ design เล่นทดสอบยืนยัน**

ให้เขียนเป็น const จุดเดียวพร้อมคอมเมนต์ที่ทำให้คนอ่านรู้ว่านี่คือค่าที่รอยืนยัน ไม่ใช่ค่าที่ตัดสินแล้ว:

```csharp
// Domain/DrinkDeviation.cs
/// <summary>
/// เพดาน deviation ที่ยังนับเป็น Seem_Like (GDD §17.3)
/// TODO(design, D7): ค่านี้ยังไม่ผ่านการเล่นทดสอบ — ตั้งตาม GDD ไว้ก่อน
/// การแก้ S1 (Σ|r−p| แทนการนับชนิด) ทำให้เกมยากขึ้นมาก ค่าจริงอาจต้องสูงกว่านี้
/// แก้ที่นี่ที่เดียว ห้าม hardcode เลข 3 ที่อื่น
/// </summary>
public const int MaxTolerance = 3;
```

**เงื่อนไขที่ต้องครบก่อนการทดสอบจะเชื่อถือได้** — ไม่งั้นตัวเลขที่ได้จะใช้กับเกมจริงไม่ได้:

| # | เงื่อนไข | เหตุผล |
|---|---|---|
| 1 | สลับซีนทดสอบให้ใช้ `Normal_Cocktail` (26 สูตร) | ซีนพัฒนาปัจจุบันชี้ไป `Demo_Normal_Cocktail` ที่มีแค่ 6 สูตร (§2.1) — จำนวนสูตรเปลี่ยนโอกาสเจอ near-match อย่างมีนัยสำคัญ |
| 2 | Phase 2 เสร็จครบ (S1 + S2 + S3) | ทดสอบก่อน S1 เสร็จ = ทดสอบสูตรเก่า ไม่มีความหมาย |
| 3 | ตรวจ `Σ = 10` ครบทุกสูตรแล้ว (S12) | สูตรที่รวมไม่ถึง 10 จะได้ deviation สูงเกินจริงตลอด |

**ตัวเลขอ้างอิงที่ควรวัดระหว่างทดสอบ** — ให้ design ลองที่ `2`, `3`, `4`, `5` แล้วดู
สัดส่วน Perfect / Acceptable / Fail ที่ผู้เล่นจริงทำได้ ค่าที่เหมาะคือค่าที่ผู้เล่นตั้งใจชงถูก
แล้วพลาดเล็กน้อยยังได้ `Acceptable` แต่ชงมั่วแล้วได้ `Fail` — ปัจจุบันชงมั่วก็ยังได้ `Acceptable` (S1)
...
## 12. GDD ที่ต้องแก้ตาม / GDD Errata
...
| 10 | D7 `MaxTolerance` | **คง `3` ตาม GDD ไว้ก่อน — ⚠️ FLAG รอ design** | ต้องเล่นทดสอบจริงหลัง Phase 2 ถึงจะรู้ค่าที่เหมาะ | §10.3 · เขียน `TODO(design, D7)` กำกับที่ const |
...
**สถานะ: ปิดครบทุกข้อ ยกเว้น D7 ที่ตั้งใจค้างไว้รอ design** — แผนพร้อมลงมือ Phase 0 ได้ทันที
